...
export const getVendorByIdNoAuth = async (vendorId: string) => {
...
export const approveVendor = async (vendorId: string) => {
  try {
    const res = await apiClient.put(
      `/super-admin/vendors/${String(vendorId)}/approve`
    );
    return res.data;
  } catch (error) {
...
export const rejectVendor = async (vendorId: number) => {
  const response = await apiClient.put(
    `/super-admin/vendors/${vendorId}/reject`
  );
  return response.data;
};
...
export const suspendVendor = async (vendorId: number) => {
  const response = await apiClient.put(
    `/super-admin/vendors/${vendorId}/suspend`
  );
  return response.data;
};
